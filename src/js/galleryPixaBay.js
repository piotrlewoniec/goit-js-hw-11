...
(() => {
  const galleryPixaBay = {
    ...
    galleryClickHandler: function (event) {
      ...
      this.instanceSimpleLightbox.on('show.simplelightbox', function (e) {
        console.log(e); // some usefull information
      });
      this.instanceSimpleLightbox.on('shown.simplelightbox', function (e) {
        console.log(e); // some usefull information
      });
      this.instanceSimpleLightbox.on('close.simplelightbox', function (e) {
        console.log(e); // some usefull information
      });
      this.instanceSimpleLightbox.on('closed.simplelightbox', function (e) {
        console.log(e); // some usefull information
      });
    ...
    apiRestAxios: function (searchString, pagination, loadMore = false) {
      ...
      async function getData() {
        try {
          const response = await axios(
            {
              ...
              // url: 'https://pixabay.com/api/?key=34529652-cc8793dd1496f54354573213f&q=yellow+flowers&image_type=photo',
              params: {
          ...
          //console.log('response.lengt', response.data.hits.length);
          if (response.data.hits.length !== 0) {
            searchedDataLocal = response.data;
            //console.log('searchDataLocal', searchedDataLocal);
          } else {
            searchedDataLocal = -1;
          }
          //console.log('searchedDataLocal', searchedDataLocal);
          if (searchedDataLocal === -1 && !loadMore) {
            ...
            console.log('searchedData', pointerObj.searchedData);
            if (pagination < 13) {
...
// axios
//   .get(
//     'https://pixabay.com/api/?key=34529652-cc8793dd1496f54354573213f&q=yellow+flowers&image_type=photo'
//   )
